...
class ReteRecWithAttention(nn.Module):
    def __init__(self, window_size, freq_bins, hidden_size=128, dropout_p=0.2):
        ...
        self.rec_imag_1 = nn.LSTM(
            self.freq_bins, hidden_size * 2, num_layers=1, batch_first=True
        )
        self.rec_real_1 = nn.LSTM(
            self.freq_bins, hidden_size * 2, num_layers=1, batch_first=True
        )

        self.rec_imag_2 = nn.LSTM(
            hidden_size * 2, hidden_size, num_layers=1, batch_first=True
        )
        self.rec_real_2 = nn.LSTM(
            hidden_size * 2, hidden_size, num_layers=1, batch_first=True
        )

        self.rec_dec_1 = nn.LSTM(
            hidden_size * 2, hidden_size * 2, num_layers=1, batch_first=True
        )
        self.rec_dec_2 = nn.LSTM(
            hidden_size * 2, hidden_size * 2, num_layers=1, batch_first=True
        )
    ...
    def forward(self, real, imag):
        ...
        out_imag = F.leaky_relu(
            self.rec_imag_2(F.leaky_relu(self.rec_imag_1(imag)[0]))[0]
        )
        out_real = F.leaky_relu(
            self.rec_real_2(F.leaky_relu(self.rec_real_1(real)[0]))[0]
        )
        ...
        decoder_input = (
            encoder_output[:, -1, :].squeeze(1)
            if encoder_output.dim() > 2
            else encoder_output[:, -1, :]
        )
        ...
        for t in range(real.size(1)):
            ...
            out_proj_real = F.leaky_relu(
                self.proiezione_real_2(
                    F.leaky_relu(self.proiezione_real_1(decoder_output))
                )
            )
            out_proj_imag = F.leaky_relu(
                self.proiezione_imag_2(
                    F.leaky_relu(self.proiezione_imag_1(decoder_output))
                )
            )
